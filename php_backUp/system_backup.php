<!--Test Oracle file for UBC CPSC304 2018 Winter Term 1
  ...
  <html>
    ...
    <body>
        ...
        <h2>DIVISION: Display Customer Who Do Not Been Served By A Staff</h2>
        <form method="GET" action="system.php"> <!--refresh page when submitted-->
            <input type="hidden" id="displayCustomerInfo" name="displayCustomerInfo">
            <input type="submit" name="displayCustomerInfo"></p>
        </form>
        ...
        function handleDivisionCustomerWhoDoNotBeenServedByAStaff() {
            global $db_conn;

            $result = executePlainSQL("SELECT c.customer_name, c.phone FROM Customer c WHERE (c.customer_name!='1') AND (c.customer_name!='456') AND NOT EXISTS (SELECT s.customer_phone FROM Serve s WHERE c.phone=s.customer_phone)");

            echo "<table>";
            echo "<tr><th>Name</th><th>PhoneNumber</th></tr>";
	...
	// A better coding practice is to have one method that reroutes your requests accordingly. It will make it easier to add/remove functionality.
        function handleGETRequest() {
            if (connectToDB()) {
                ...
                } else if (array_key_exists('displayCustomerInfo', $_GET)) {
                    handleDivisionCustomerWhoDoNotBeenServedByAStaff();
		...
		if (isset($_POST['reset']) || isset($_POST['updateSubmit']) || isset($_POST['insertSubmit'])) {
            ...
            handleGETRequest();
        } else if (isset($_GET['displayCustomerInfo'])) {
            handleGETRequest();
